<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dayList">
      <el-table-column label="日期时间" align="center" prop="data_time" />
      <el-table-column label="游戏产品" align="center" prop="productName" />
      <el-table-column label="场景值" align="center" prop="scene_code" />
      <el-table-column label="注册人数" align="center" prop="register_count" />
      <el-table-column label="注册次留人数" align="center" prop="register_one_count" />
      <el-table-column label="注册次留" align="center" prop="register_one_retain" />
      <el-table-column label="注册三留人数" align="center" prop="register_three_count" />
       <el-table-column label="注册三留" align="center" prop="register_three_retain" />
      <el-table-column label="注册七留人数" align="center" prop="register_seven_count" />
      <el-table-column label="注册七留" align="center" prop="register_seven_retain" />
    </el-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      request({
        url: '/business/reportSceneDay/queryCategoryData',
        method: 'post',
        data:JSON.stringify(this.getQueryData())
      }).then(
        response => {
          console.log("response.data.recodeList",response.data.recordList);
          this.dayList = response.data.recordList;
    ...
    exportRetain(){
        ...
        downLoadZip(encodeURI("/business/reportSceneDay/gameExportRetainData?filterJson=" + filterJsonStr), "exportShare",true);
    }
  }
};
</script>
