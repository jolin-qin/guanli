<template>
  <div class="app-container">
   ...
   <el-row :gutter="10" class="mb8 qphead">
    ...
    <el-table v-loading="loading" :data="materialReportList" >
      <el-table-column label="日期" prop="createTime" />
      <el-table-column label="素材位置" prop="locationCode"  />
      <el-table-column label="活跃用户" prop="activeCount"  />
      <el-table-column label="点击次数" prop="clickCount"  />
      <el-table-column label="弹框弹出次数" prop="displayCount"  />
      <el-table-column label="导出成功次数" prop="exportCount"  />
      <el-table-column label="位置显示率" prop="displayRate"/>
      <el-table-column label="位置点击率" prop="clickRate"  />
      <el-table-column label="位置导出率" prop="exportRate" />
    </el-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.materialList().then(
        response => {
          this.materialReportList = response.data;
          this.loading = false;
        }
      );
    },
    materialList(){
      return request({
        url: '/reportGameQuery/queryMaterialData',
        method: 'post',
    ...
    handleExport(){
        ...
        downLoadZip(encodeURI("/reportGameExport/gameMaterialExport?filterJson=" + filterJsonStr), "exportCategory",true);
    },
   //产品下拉
